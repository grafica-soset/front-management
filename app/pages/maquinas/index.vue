...
/**
 * Listagem paginada das impressoras offset da empresa ativa
 * (GET /printing-machines/page). Filtro por status ativo.
 */
...
import { MACHINE_TYPE_LABELS, PRINTING_MACHINES_BASE } from '@/utils/machineCatalog'
import type { MachinePageItem } from '@/types/Machine'

definePageMeta({ middleware: 'auth' })

const auth = useAuthStore()
const toast = useToast()
const { listPage, remove } = useMachines(PRINTING_MACHINES_BASE)
...
<template>
  <div class="space-y-6">
    <header class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
      <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Máquinas — Impressão Offset</h1>
