...
/**
 * Cadastro de nova máquina de grampeadeira. Requer empresa ativa (o `customerId` é preenchido
 * pelo composable a partir dela). Com `?duplicate=<id>`, pré-preenche a partir da origem.
 */
import { computed, ref } from 'vue'
import StitchingMachineForm from '@/components/forms/StitchingMachineForm.vue'
import { useMachines } from '@/composables/useMachines'
import { useToast } from '@/composables/useToast'
import { useAuthStore } from '@/stores/auth'
import { extractApiError } from '@/utils/apiError'
import { STITCHING_MACHINES_BASE } from '@/utils/machineCatalog'
import type { StitchingMachine, StitchingMachineRequest } from '@/types/Machine'
...
const { getById, create } = useMachines<StitchingMachine, StitchingMachineRequest>(STITCHING_MACHINES_BASE)
...
const prefill = ref<StitchingMachine | null>(null)
const prefilling = ref(isDuplicating)
const prefillError = ref<string | null>(null)

const heading = computed(() =>
  isDuplicating ? 'Duplicar máquina de grampeadeira' : 'Nova máquina de grampeadeira',
)
...
const handleSubmit = async (payload: StitchingMachineRequest) => {
  if (!auth.activeCompanyId) {
    serverError.value = 'Selecione uma empresa antes de cadastrar máquinas de grampeadeira.'
    return
  }
  loading.value = true
  ...
  try {
    ...
    await navigateTo('/maquinas/grampeadeira')
  } catch (err) {
    serverError.value = extractApiError(err, 'Falha ao cadastrar a máquina de grampeadeira.')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="space-y-6">
    <header>
      <NuxtLink to="/maquinas/grampeadeira" class="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200">
    ...
    <div v-if="!auth.activeCompanyId" class="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800 dark:bg-amber-900/30 dark:border-amber-800 dark:text-amber-200">
      Selecione uma empresa antes de cadastrar máquinas de grampeadeira.
    </div>
    ...
    <div v-else class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 dark:bg-slate-800 dark:border-slate-700">
      <StitchingMachineForm
        mode="create"
        :initial="prefill"
        :loading="loading"
        :server-error="serverError"
        @submit="handleSubmit"
        @cancel="navigateTo('/maquinas/grampeadeira')"
      />
    </div>
  </div>
</template>
