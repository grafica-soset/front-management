...
/**
 * Listagem paginada das máquinas de grampeadeira da empresa ativa
 * (GET /stitching-machines/page). Filtro por status ativo.
 */
...
import { MACHINE_TYPE_LABELS, STITCHING_MACHINES_BASE } from '@/utils/machineCatalog'
import type { StitchingMachine, StitchingMachineRequest, MachinePageItem } from '@/types/Machine'

definePageMeta({ middleware: 'auth' })

const auth = useAuthStore()
const toast = useToast()
const { listPage, remove } = useMachines<StitchingMachine, StitchingMachineRequest>(STITCHING_MACHINES_BASE)
...
const refresh = async () => {
  ...
  try {
    ...
    listError.value = extractApiError(err, 'Falha ao carregar as máquinas de grampeadeira.')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="space-y-6">
    <header class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
      <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Máquinas — Grampeadeira</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Parque de grampeadeira de <span class="font-medium">{{ auth.activeCompany?.value ?? '—' }}</span>.
        </p>
      </div>
      <NuxtLink
        v-if="hasCompany"
        to="/maquinas/grampeadeira/nova"
    ...
    <div v-if="!hasCompany" class="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800 dark:bg-amber-900/30 dark:border-amber-800 dark:text-amber-200">
      Selecione uma empresa para ver e gerenciar as máquinas de grampeadeira.
    </div>
    ...
    <template v-else>
      ...
      <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden dark:bg-slate-800 dark:border-slate-700">
        ...
        <div v-else-if="items.length === 0" class="p-8 text-sm text-center text-slate-500 dark:text-slate-400">
          Nenhuma máquina de grampeadeira cadastrada.
        </div>
              ...
              <tr v-for="item in items" :key="item.id" class="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors">
                ...
                <td class="px-5 py-3 text-right">
                  <div class="inline-flex items-center gap-1">
                    <NuxtLink
                      :to="`/maquinas/grampeadeira/${item.id}/editar`"
                    ...
                    <NuxtLink
                      :to="`/maquinas/grampeadeira/nova?duplicate=${item.id}`"
