...
/**
 * Edição de máquina de furadeira. Busca a máquina completa
 * (GET /hole-punching-machines/{id}) para popular o formulário e salva via PUT.
 */
import { ref } from 'vue'
...
const { getById, update } = useMachines<HolePunchingMachine, HolePunchingMachineRequest>(HOLE_PUNCHING_MACHINES_BASE)

const machine = ref<HolePunchingMachine | null>(null)
...
const handleSubmit = async (payload: HolePunchingMachineRequest) => {
  ...
  try {
    ...
    serverError.value = extractApiError(err, 'Falha ao atualizar a máquina de furadeira.')
  } finally {
    saving.value = false
  }
}
  ...
  <div class="space-y-6">
    <header>
      ...
      <h1 class="mt-3 text-2xl font-bold text-slate-900 dark:text-white">
        Editar furadeira<span v-if="machine"> — {{ machine.name }}</span>
      </h1>
    </header>
    ...
    <div v-else-if="machine" class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 dark:bg-slate-800 dark:border-slate-700">
      <HolePunchingMachineForm mode="edit" :initial="machine" :loading="saving" :server-error="serverError" @submit="handleSubmit" @cancel="navigateTo('/maquinas/furadeira')" />
    </div>
  </div>
</template>
