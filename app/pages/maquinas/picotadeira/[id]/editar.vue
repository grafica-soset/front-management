...
/**
 * Edição de máquina de picotadeira. Busca a máquina completa (GET /perforating-machines/{id})
...
const { getById, update } = useMachines<PerforatingMachine, PerforatingMachineRequest>(PERFORATING_MACHINES_BASE)

const machine = ref<PerforatingMachine | null>(null)
...
const handleSubmit = async (payload: PerforatingMachineRequest) => {
  ...
  try {
    ...
    serverError.value = extractApiError(err, 'Falha ao atualizar a máquina de picotadeira.')
  } finally {
    saving.value = false
  }
}
  ...
  <div class="space-y-6">
    <header>
      ...
      <h1 class="mt-3 text-2xl font-bold text-slate-900 dark:text-white">
        Editar picotadeira<span v-if="machine"> — {{ machine.name }}</span>
      </h1>
    </header>
    ...
    <div v-else-if="machine" class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 dark:bg-slate-800 dark:border-slate-700">
      <PerforatingMachineForm mode="edit" :initial="machine" :loading="saving" :server-error="serverError" @submit="handleSubmit" @cancel="navigateTo('/maquinas/picotadeira')" />
    </div>
  </div>
</template>
