...
/**
 * Cadastro de nova impressora digital. Requer empresa ativa. Com `?duplicate=<id>`, pré-preenche
 * a partir da máquina de origem.
 */
import { computed, ref } from 'vue'
import DigitalMachineForm from '@/components/forms/DigitalMachineForm.vue'
import { useMachines } from '@/composables/useMachines'
import { useToast } from '@/composables/useToast'
import { useAuthStore } from '@/stores/auth'
import { extractApiError } from '@/utils/apiError'
import { DIGITAL_MACHINES_BASE } from '@/utils/machineCatalog'
import type { DigitalMachine, DigitalMachineRequest } from '@/types/Machine'
...
const { getById, create } = useMachines<DigitalMachine, DigitalMachineRequest>(DIGITAL_MACHINES_BASE)
...
const prefill = ref<DigitalMachine | null>(null)
const prefilling = ref(isDuplicating)
const prefillError = ref<string | null>(null)

const heading = computed(() =>
  isDuplicating ? 'Duplicar impressora digital' : 'Nova impressora digital',
)
...
const handleSubmit = async (payload: DigitalMachineRequest) => {
  if (!auth.activeCompanyId) {
    serverError.value = 'Selecione uma empresa antes de cadastrar impressoras digitais.'
    return
  }
  loading.value = true
  ...
  try {
    ...
    await navigateTo('/maquinas/digital')
  } catch (err) {
    serverError.value = extractApiError(err, 'Falha ao cadastrar a impressora digital.')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="space-y-6">
    <header>
      <NuxtLink to="/maquinas/digital" class="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200">
    ...
    <div v-if="!auth.activeCompanyId" class="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800 dark:bg-amber-900/30 dark:border-amber-800 dark:text-amber-200">
      Selecione uma empresa antes de cadastrar impressoras digitais.
    </div>
    ...
    <div v-else class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 dark:bg-slate-800 dark:border-slate-700">
      <DigitalMachineForm
        ...
        @submit="handleSubmit"
        @cancel="navigateTo('/maquinas/digital')"
      />
    </div>
  </div>
</template>
