...
/**
 * Listagem paginada das guilhotinas da empresa ativa (GET /cutting-machines/page).
...
import { MACHINE_TYPE_LABELS, CUTTING_MACHINES_BASE } from '@/utils/machineCatalog'
import type { CuttingMachine, CuttingMachineRequest, MachinePageItem } from '@/types/Machine'

definePageMeta({ middleware: 'auth' })

const auth = useAuthStore()
const toast = useToast()
const { listPage, remove } = useMachines<CuttingMachine, CuttingMachineRequest>(CUTTING_MACHINES_BASE)
...
const refresh = async () => {
  ...
  try {
    ...
    listError.value = extractApiError(err, 'Falha ao carregar as guilhotinas.')
  } finally {
    loading.value = false
  }
}
...
const handleDelete = async (item: MachinePageItem) => {
  if (!window.confirm(`Remover a guilhotina "${item.name}"? Esta ação não pode ser desfeita.`)) return
  deletingId.value = item.id
  try {
    await remove(item.id)
    toast.success(`Guilhotina "${item.name}" removida.`)
    if (items.value.length === 1 && page.value > 0) page.value -= 1
    await refresh()
  } catch (err) {
    toast.error(extractApiError(err, 'Não foi possível remover a guilhotina.'))
  } finally {
...
<template>
  <div class="space-y-6">
    <header class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
      <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Máquinas — Guilhotina</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Parque de corte de <span class="font-medium">{{ auth.activeCompany?.value ?? '—' }}</span>.
        </p>
      </div>
      <NuxtLink
        v-if="hasCompany"
        to="/maquinas/corte/nova"
    ...
    <div v-if="!hasCompany" class="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800 dark:bg-amber-900/30 dark:border-amber-800 dark:text-amber-200">
      Selecione uma empresa para ver e gerenciar as guilhotinas.
    </div>
    ...
    <template v-else>
      ...
      <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden dark:bg-slate-800 dark:border-slate-700">
        <div v-if="loading" class="p-6 text-sm text-slate-500 dark:text-slate-400">Carregando guilhotinas...</div>
              ...
              <tr v-for="item in items" :key="item.id" class="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors">
                ...
                <td class="px-5 py-3 text-right">
                  <div class="inline-flex items-center gap-1">
                    <NuxtLink
                      :to="`/maquinas/corte/${item.id}/editar`"
                    ...
                    <NuxtLink
                      :to="`/maquinas/corte/nova?duplicate=${item.id}`"
        ...
        <div v-if="!loading && !listError && totalPages > 1" class="flex items-center justify-between px-5 py-3 border-t border-slate-100 dark:border-slate-700/50">
          <span class="text-xs text-slate-500 dark:text-slate-400">{{ totalItems }} guilhotina(s) — página {{ page + 1 }} de {{ totalPages }}</span>
