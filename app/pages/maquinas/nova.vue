...
/**
 * Cadastro de nova impressora offset. Requer empresa ativa (o `customerId` é
 * preenchido pelo composable a partir dela).
 *
 * Quando recebe `?duplicate=<id>`, busca a máquina de origem e pré-preenche o
 * formulário com todos os parâmetros dela (modo criação), sugerindo um nome
 * "… (cópia)" — assim o usuário revisa/ajusta antes de salvar como uma nova
 * máquina. Não há endpoint de duplicação: reusamos GET /{id} + POST.
 */
...
const { getById, create } = useMachines(PRINTING_MACHINES_BASE)

const loading = ref(false)
const serverError = ref<string | null>(null)

// Duplicação: id da máquina de origem (quando presente na query).
const duplicateId = Number(route.query.duplicate)
const isDuplicating = Number.isFinite(duplicateId) && duplicateId > 0

const prefill = ref<Machine | null>(null)
const prefilling = ref(isDuplicating)
const prefillError = ref<string | null>(null)

const heading = computed(() =>
  isDuplicating ? 'Duplicar máquina — Impressão Offset' : 'Nova máquina — Impressão Offset',
)
...
onMounted(async () => {
  ...
  try {
    ...
    // Sugere um nome distinto para não colidir com a máquina de origem (409).
