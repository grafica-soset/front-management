...
const { create } = useMachines(PRINTING_MACHINES_BASE)
...
<template>
  <div class="space-y-6">
    <header>
      ...
      <h1 class="mt-3 text-2xl font-bold text-slate-900 dark:text-white">Nova máquina — Impressão Offset</h1>
    </header>
    ...
    <div v-else class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 dark:bg-slate-800 dark:border-slate-700">
      <MachineForm :loading="loading" :server-error="serverError" @submit="handleSubmit" @cancel="navigateTo('/maquinas')" />
    </div>
  </div>
</template>
