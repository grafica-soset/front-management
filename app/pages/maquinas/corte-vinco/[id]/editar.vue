...
/**
 * Edição de máquina de corte e vinco. Busca a máquina completa
 * (GET /die-cutting-machines/{id}) para popular o formulário e salva via PUT.
 */
import { ref } from 'vue'
...
const handleSubmit = async (payload: DieCuttingMachineRequest) => {
  ...
  try {
    ...
    serverError.value = extractApiError(err, 'Falha ao atualizar a máquina de corte e vinco.')
  } finally {
    saving.value = false
  }
}
  ...
  <div class="space-y-6">
    <header>
      ...
      <h1 class="mt-3 text-2xl font-bold text-slate-900 dark:text-white">
        Editar corte e vinco<span v-if="machine"> — {{ machine.name }}</span>
      </h1>
    </header>
