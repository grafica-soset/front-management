...
/**
 * Cadastro de nova máquina de plastificadora. Requer empresa ativa (o `customerId` é preenchido
...
const { getById, create } = useMachines<LaminatingMachine, LaminatingMachineRequest>(LAMINATING_MACHINES_BASE)
...
const prefill = ref<LaminatingMachine | null>(null)
const prefilling = ref(isDuplicating)
const prefillError = ref<string | null>(null)

const heading = computed(() =>
  isDuplicating ? 'Duplicar máquina de plastificadora' : 'Nova máquina de plastificadora',
)
...
const handleSubmit = async (payload: LaminatingMachineRequest) => {
  if (!auth.activeCompanyId) {
    serverError.value = 'Selecione uma empresa antes de cadastrar máquinas de plastificadora.'
    return
  }
  loading.value = true
  ...
  try {
    ...
    serverError.value = extractApiError(err, 'Falha ao cadastrar a máquina de plastificadora.')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="space-y-6">
    ...
    <div v-if="!auth.activeCompanyId" class="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800 dark:bg-amber-900/30 dark:border-amber-800 dark:text-amber-200">
      Selecione uma empresa antes de cadastrar máquinas de plastificadora.
    </div>
    ...
    <div v-else class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 dark:bg-slate-800 dark:border-slate-700">
      <LaminatingMachineForm
        mode="create"
        :initial="prefill"
        :loading="loading"
        :server-error="serverError"
        @submit="handleSubmit"
        @cancel="navigateTo('/maquinas/plastificadora')"
      />
    </div>
  </div>
</template>
