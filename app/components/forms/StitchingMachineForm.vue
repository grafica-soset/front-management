...
/**
 * Formulário da GRAMPEADEIRA (categoria FINISHING): identificação + formato de papel + alimentação
 * (altura da pilha) + custo-hora + transporte de insumos + bloco grampeadeira.
 *
 * A grampeadeira grampeia blocos a arame; a bancada (área de manuseio) define quantos blocos são
 * grampeados de uma vez. NÃO há rampa de velocidade nem margem da pinça. O formato é editado na
 * unidade da empresa e convertido para milímetros no envio.
 */
import { computed, reactive, ref } from 'vue'
import type { StitchingBlockRequest, StitchingMachine, StitchingMachineRequest } from '@/types/Machine'
import {
  MACHINE_TYPE_LABELS,
  defaultStitchingBlock,
  hydrateStitchingBlock,
  validateStitching,
...
const emit = defineEmits<{
  (e: 'submit', payload: StitchingMachineRequest, mode: 'create' | 'update'): void
  (e: 'cancel'): void
}>()
...
const stitching = reactive<StitchingBlockRequest>(defaultStitchingBlock())

const commonErrors = ref<Record<string, string>>({})
const stitchingErrors = ref<Record<string, string>>({})

if (props.initial) hydrate(props.initial)

function hydrate(machine: StitchingMachine) {
  ...
  Object.assign(stitching, hydrateStitchingBlock(machine.stitching))
}
...
const handleSubmit = () => {
  ...
  stitchingErrors.value = validateStitching(stitching)

  if (Object.keys(commonErrors.value).length || Object.keys(stitchingErrors.value).length) return

  const payload: StitchingMachineRequest = {
    customerId: 0,
    ...
    stitching: JSON.parse(JSON.stringify(stitching)) as StitchingBlockRequest,
  }
  ...
  <form @submit.prevent="handleSubmit" class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        ...
        <input
          :value="MACHINE_TYPE_LABELS.STITCHING"
          type="text"
          readonly
          tabindex="-1"
      ...
      <div class="md:col-span-2">
        ...
        <input v-model="form.name" type="text" maxlength="150" placeholder="Ex.: Grampeadeira" :class="inputClass('name')" />
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <fieldset class="rounded-lg border border-slate-200 p-4 min-w-0 dark:border-slate-700">
        ...
        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Altura de papel na mesa de alimentação.</p>
    ...
    <!-- Bloco grampeadeira -->
    <fieldset class="rounded-lg border border-slate-200 p-4 min-w-0 dark:border-slate-700">
      <legend class="px-2 text-sm font-semibold text-slate-700 dark:text-slate-200">Grampeadeira</legend>
      <StitchingBlockFields :block="stitching" :errors="stitchingErrors" />
    </fieldset>
    ...
    <div class="flex justify-end gap-3 pt-2">
      ...
      >
        {{ loading ? 'Salvando...' : isEditing ? 'Salvar alterações' : 'Cadastrar grampeadeira' }}
      </button>
    </div>
  </form>
</template>
