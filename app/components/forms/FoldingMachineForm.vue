...
/**
 * Formulário da DOBRADEIRA (categoria FINISHING): identificação + formato de papel + alimentação
 * (altura da pilha) + custo-hora + transporte de insumos + bloco dobradeira.
 *
 * A dobradeira dobra por bolsas (paralelas e cruzadas); a velocidade parte da máxima e é reduzida
 * por bolsa/gramatura/formato. NÃO há margem da pinça. O formato é editado na unidade da empresa.
 */
import { computed, reactive, ref } from 'vue'
import type { FoldingBlockRequest, FoldingMachine, FoldingMachineRequest } from '@/types/Machine'
import {
  MACHINE_TYPE_LABELS,
  defaultFoldingBlock,
  hydrateFoldingBlock,
...
const emit = defineEmits<{
  (e: 'submit', payload: FoldingMachineRequest, mode: 'create' | 'update'): void
  (e: 'cancel'): void
}>()
...
const folding = reactive<FoldingBlockRequest>(defaultFoldingBlock())

const commonErrors = ref<Record<string, string>>({})
const foldingErrors = ref<Record<string, string>>({})

if (props.initial) hydrate(props.initial)

function hydrate(machine: FoldingMachine) {
  ...
  Object.assign(folding, hydrateFoldingBlock(machine.folding))
}
...
const handleSubmit = () => {
  ...
  foldingErrors.value = validateFolding(folding)

  // Formato ideal máximo ≥ mínimo (pelo tamanho linear).
  const minSize = folding.minFormat.widthMm + folding.minFormat.lengthMm
  const maxSize = folding.maxFormat.widthMm + folding.maxFormat.lengthMm
  if (maxSize < minSize) {
    foldingErrors.value['maxFormat.widthMm'] = 'O formato ideal máximo deve ser ≥ o mínimo.'
  }

  if (Object.keys(commonErrors.value).length || Object.keys(foldingErrors.value).length) return

  const payload: FoldingMachineRequest = {
    customerId: 0,
    machineType: 'FOLDING',
    name: form.name.trim(),
    formatRange: {
    ...
    folding: JSON.parse(JSON.stringify(folding)) as FoldingBlockRequest,
  }
  ...
  <form @submit.prevent="handleSubmit" class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        ...
        <input
          :value="MACHINE_TYPE_LABELS.FOLDING"
          type="text"
          readonly
          tabindex="-1"
      ...
      <div class="md:col-span-2">
        ...
        <input v-model="form.name" type="text" maxlength="150" placeholder="Ex.: Dobradeira" :class="inputClass('name')" />
    ...
    <!-- Bloco dobradeira -->
    <fieldset class="rounded-lg border border-slate-200 p-4 min-w-0 dark:border-slate-700">
      <legend class="px-2 text-sm font-semibold text-slate-700 dark:text-slate-200">Dobradeira</legend>
      <FoldingBlockFields :block="folding" :errors="foldingErrors" />
    </fieldset>
    ...
    <div class="flex justify-end gap-3 pt-2">
      ...
      >
        {{ loading ? 'Salvando...' : isEditing ? 'Salvar alterações' : 'Cadastrar dobradeira' }}
      </button>
    </div>
  </form>
</template>
