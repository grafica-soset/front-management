...
/**
 * Formulário da FURADEIRA (categoria FINISHING): identificação + formato de papel + alimentação
 * (altura da pilha) + custo-hora + transporte de insumos + bloco furadeira (tempos por furo/descida).
 *
 * A furadeira se parece com a guilhotina: NÃO há margens, cores, quebras nem rampa. O formato de
 * papel é editado na unidade da empresa ativa e convertido para milímetros no envio.
 */
import { computed, reactive, ref } from 'vue'
import type { HolePunchingBlock, HolePunchingMachine, HolePunchingMachineRequest } from '@/types/Machine'
import {
  MACHINE_TYPE_LABELS,
...
import HolePunchingBlockFields from '@/components/forms/machines/HolePunchingBlock.vue'
import { useUnitConverter } from '@/composables/useUnitConverter'

const props = defineProps<{
  /** Dados para pré-preencher o formulário (edição ou duplicação). */
  initial?: HolePunchingMachine | null
...
const emit = defineEmits<{
  (e: 'submit', payload: HolePunchingMachineRequest, mode: 'create' | 'update'): void
  (e: 'cancel'): void
}>()
...
const holePunching = reactive<HolePunchingBlock>(defaultHolePunchingBlock())

const commonErrors = ref<Record<string, string>>({})
const holePunchingErrors = ref<Record<string, string>>({})

if (props.initial) hydrate(props.initial)

function hydrate(machine: HolePunchingMachine) {
  ...
  Object.assign(holePunching, hydrateHolePunchingBlock(machine.holePunching))
}
...
const handleSubmit = () => {
  ...
  holePunchingErrors.value = validateHolePunching(holePunching)

  if (Object.keys(commonErrors.value).length || Object.keys(holePunchingErrors.value).length) return

  const payload: HolePunchingMachineRequest = {
    customerId: 0,
    ...
    holePunching: JSON.parse(JSON.stringify(holePunching)) as HolePunchingBlock,
  }
  ...
  <form @submit.prevent="handleSubmit" class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        ...
        <input
          :value="MACHINE_TYPE_LABELS.HOLE_PUNCHING"
          type="text"
          readonly
          tabindex="-1"
    ...
    <!-- Bloco furadeira -->
    <fieldset class="rounded-lg border border-slate-200 p-4 min-w-0 dark:border-slate-700">
      <legend class="px-2 text-sm font-semibold text-slate-700 dark:text-slate-200">Furadeira</legend>
      <HolePunchingBlockFields :block="holePunching" :errors="holePunchingErrors" />
    </fieldset>
    ...
    <div class="flex justify-end gap-3 pt-2">
      ...
      >
        {{ loading ? 'Salvando...' : isEditing ? 'Salvar alterações' : 'Cadastrar furadeira' }}
      </button>
    </div>
  </form>
</template>
