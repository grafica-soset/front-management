...
/**
 * Formulário da PICOTADEIRA / SERRILHADEIRA (categoria FINISHING): identificação + formato de papel
 * + alimentação (altura da pilha) + custo-hora + transporte de insumos + bloco picotadeira.
 *
 * A picotadeira faz picotes; cada picote usa uma ferramenta. A velocidade parte da máxima e é
 * reduzida por gramatura/formato. NÃO há margem da pinça. O formato é editado na unidade da empresa.
 */
import { computed, reactive, ref } from 'vue'
import type { PerforatingBlockRequest, PerforatingMachine, PerforatingMachineRequest } from '@/types/Machine'
import {
  MACHINE_TYPE_LABELS,
  defaultPerforatingBlock,
  hydratePerforatingBlock,
  validatePerforating,
} from '@/utils/machineCatalog'
import PerforatingBlockFields from '@/components/forms/machines/PerforatingBlock.vue'
import { useUnitConverter } from '@/composables/useUnitConverter'

const props = defineProps<{
  initial?: PerforatingMachine | null
  mode?: 'create' | 'edit'
  loading?: boolean
...
const emit = defineEmits<{
  (e: 'submit', payload: PerforatingMachineRequest, mode: 'create' | 'update'): void
  (e: 'cancel'): void
}>()
...
const perforating = reactive<PerforatingBlockRequest>(defaultPerforatingBlock())

const commonErrors = ref<Record<string, string>>({})
const perforatingErrors = ref<Record<string, string>>({})

if (props.initial) hydrate(props.initial)

function hydrate(machine: PerforatingMachine) {
  ...
  Object.assign(perforating, hydratePerforatingBlock(machine.perforating))
}
...
const handleSubmit = () => {
  ...
  perforatingErrors.value = validatePerforating(perforating)

  // Formato ideal máximo ≥ mínimo (pelo tamanho linear).
  const minSize = perforating.minFormat.widthMm + perforating.minFormat.lengthMm
  const maxSize = perforating.maxFormat.widthMm + perforating.maxFormat.lengthMm
  if (maxSize < minSize) {
    perforatingErrors.value['maxFormat.widthMm'] = 'O formato ideal máximo deve ser ≥ o mínimo.'
  }

  if (Object.keys(commonErrors.value).length || Object.keys(perforatingErrors.value).length) return

  const payload: PerforatingMachineRequest = {
    customerId: 0,
    ...
    perforating: JSON.parse(JSON.stringify(perforating)) as PerforatingBlockRequest,
  }
  ...
  <form @submit.prevent="handleSubmit" class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        ...
        <input
          :value="MACHINE_TYPE_LABELS.PERFORATING"
          type="text"
          readonly
          tabindex="-1"
      ...
      <div class="md:col-span-2">
        ...
        <input v-model="form.name" type="text" maxlength="150" placeholder="Ex.: Baunmak" :class="inputClass('name')" />
    ...
    <!-- Bloco picotadeira -->
    <fieldset class="rounded-lg border border-slate-200 p-4 min-w-0 dark:border-slate-700">
      <legend class="px-2 text-sm font-semibold text-slate-700 dark:text-slate-200">Picotadeira</legend>
      <PerforatingBlockFields :block="perforating" :errors="perforatingErrors" />
    </fieldset>
    ...
    <div class="flex justify-end gap-3 pt-2">
      ...
      >
        {{ loading ? 'Salvando...' : isEditing ? 'Salvar alterações' : 'Cadastrar picotadeira' }}
      </button>
    </div>
  </form>
</template>
