...
// BANDEJA DE SAÍDA (atividade 035): é ela que diz de quantas em quantas folhas o operador para
// para tirar o papel picotado. Antes o orçamento supunha 10 cm para todas as máquinas.
const outputTrayHeight = dimensionModel(
  () => props.block.outputTrayHeightMm,
  (mm) => (props.block.outputTrayHeightMm = mm),
)
  ...
  <div class="space-y-6">
    ...
    <fieldset class="rounded-lg border border-slate-200 p-4 min-w-0 dark:border-slate-700">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <label class="block mb-2 text-sm text-slate-700 dark:text-slate-300">Bandeja de saída</label>
          <div class="relative">
            <input v-model.number="outputTrayHeight" type="number" min="0" step="0.001" :class="inputClass(errors.outputTrayHeightMm)" />
            <span class="absolute inset-y-0 right-3 flex items-center text-xs text-slate-500">{{ lengthUnit }}</span>
          </div>
          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
            Altura da bandeja que recebe o papel picotado — quando ela enche, alguém para e tira.
          </p>
          <p v-if="errors.outputTrayHeightMm" class="mt-1 text-xs text-rose-600">{{ errors.outputTrayHeightMm }}</p>
        </div>
        <div>
          <label class="block mb-2 text-sm text-slate-700 dark:text-slate-300">Retirada na saída</label>
          <div class="relative">
            <input v-model.number="block.outputRemovalMinutesPerTray" type="number" min="0" step="1" :class="inputClass(errors.outputRemovalMinutesPerTray)" />
            <span class="absolute inset-y-0 right-3 flex items-center text-xs text-slate-500">min</span>
          </div>
          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Tempo para esvaziar a bandeja uma vez.</p>
          <p v-if="errors.outputRemovalMinutesPerTray" class="mt-1 text-xs text-rose-600">{{ errors.outputRemovalMinutesPerTray }}</p>
        </div>
      </div>
    </fieldset>
  </div>
